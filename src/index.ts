...
async function getPostmortemArticles(query: string = "incident") {
    try {
        const response = await fetch(
            `https://vercel.com/blog?search=${encodeURIComponent(query)}`
        );
        const html = await response.text();
        const $ = cheerio.load(html);
...
async function getCustomerCases(filter?: string) {
    try {
        ...
        // 顧客事例のカードやセクションを抽出
        $('[class*="customer"], [class*="case-study"], article').each(
            (i, elem) => {
                const company = $(elem)
        ...
        tools: [
            {
                ...
                description:
                    "Vercel Statusページからインシデント履歴を取得します。最近の障害情報や現在進行中の問題を確認できます。",
                inputSchema: {
                    type: "object",
                    properties: {},
                },
            },
            {
                ...
                description:
                    "Vercel公式ブログから障害レポートやポストモーテム記事を検索します。過去のインシデント分析や対応策を学べます。",
                inputSchema: {
                    type: "object",
                    properties: {
                        query: {
                            type: "string",
                            description:
                                "検索キーワード（デフォルト: 'incident'）。'outage', 'postmortem', 'downtime'なども有効です",
                            default: "incident",
                        },
                    },
                },
            },
            {
                ...
                description:
                    "Vercelの顧客事例を取得します。企業名でフィルタリングも可能です。",
                inputSchema: {
                    type: "object",
                    properties: {
                        filter: {
